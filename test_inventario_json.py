...
def test_guardar_y_cargar_inventario(tmp_path):
    ...
    data = [{"id": 1, "nombre": "Flor", "cantidad": 5, "precio": 3000, "categoria": "Rosas"}]
...
def test_agregar_producto(monkeypatch):
    ...
    monkeypatch.setattr("rich.prompt.FloatPrompt.ask", lambda *a, **kw: float(next(inputs)))
...
def test_vender_producto(monkeypatch):
    ...
    assert resultado[0]["cantidad"] == 3  # 5 - 2 = 3
...
def test_vender_producto_stock_insuficiente(monkeypatch):
    ...
    inventario_actual = [{"id": 1, "nombre": "Rosa", "cantidad": 2, "precio": 1000, "categoria": "Flores"}]

    inputs = iter(["1", "5"])  # intenta vender más de lo que hay
    monkeypatch.setattr("rich.prompt.IntPrompt.ask", lambda *a, **kw: int(next(inputs)))

    resultado = Inventario_json.vender_producto(inventario_actual)
    assert resultado[0]["cantidad"] == 2  # sin cambios


def test_editar_producto(monkeypatch):
    """Edita correctamente los datos de un producto."""
    inventario_actual = [{"id": 1, "nombre": "Rosa", "cantidad": 10, "precio": 1000, "categoria": "Flores"}]
...
def test_buscar_producto(monkeypatch):
    ...
    inventario_actual = [
        {"id": 1, "nombre": "Rosa", "cantidad": 5, "precio": 1000, "categoria": "Flores"},
        {"id": 2, "nombre": "Maceta", "cantidad": 3, "precio": 5000, "categoria": "Decoración"},
    ]
