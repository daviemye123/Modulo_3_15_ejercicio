...
def test_suma_numeros():
    ...
    assert resultado == 15, f"Se esperaba 15, pero se obtuvo {resultado}"


def test_concatenar_strings():
    """Verifica la concatenación de strings."""
    resultado = concatenar_strings()
    assert resultado == "Hola SENA!", f"Se esperaba 'Hola SENA!', pero se obtuvo '{resultado}'"
...
def test_ejemplos_adicionales():
    ...
    assert resultado["maximo"] == 41, "El máximo debe ser 41"
    assert resultado["producto"] == 24, "El producto debe ser 24 (2×3×4)"
...
def test_funciones_no_lanzan_excepciones(funcion):
    ...
    try:
        ...
        pytest.fail(f"La función {funcion.__name__} lanzó una excepción inesperada: {e}")
