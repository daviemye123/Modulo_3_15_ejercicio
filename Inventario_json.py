...
def cargar_inventario(archivo='inventario.json'):
    ...
    if os.path.exists(archivo):
        try:
            ...
            console.print(f"[green]✓[/green] Inventario cargado: {len(inventario)} productos", style="dim")
            return inventario
        except json.JSONDecodeError:
            console.print("[yellow]⚠[/yellow] Archivo corrupto. Creando nuevo inventario.", style="dim")
            return []
    else:
        console.print("[yellow]⚠[/yellow] Archivo no encontrado. Creando nuevo inventario.", style="dim")
        return []
...
def agregar_producto(inventario):
    ...
    console.print(f"\n[green]✓[/green] Producto '{nombre}' agregado con ID: {nuevo_id}", style="bold green")
    return inventario
...
def vender_producto(inventario):
    ...
    """
    if not inventario:
        console.print("[yellow]⚠[/yellow] No hay productos en el inventario", style="bold yellow")
        return inventario
...
def editar_producto(inventario):
    ...
    guardar_inventario(inventario)
    console.print("[green]✓[/green] Producto actualizado exitosamente", style="bold green")
...
[bold red]6.[/bold red] Salir
    """
    console.print(Panel(menu, title="[bold white]MENÚ PRINCIPAL[/bold white]",
...
def main():
    ...
    while True:
        mostrar_menu()
        opcion = Prompt.ask("\n[bold]Selecciona una opción[/bold]", choices=["1", "2", "3", "4", "5", "6"])
